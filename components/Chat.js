...
// Message component to handle different types of messages including token displays
const Message = ({ message }) => {
  const getMessageClasses = (role) => {
    const baseClasses = "max-w-[80%] p-3 rounded-lg";
    switch (role) {
      case "user":
      ...
      default:
        return `${baseClasses} bg-gray-200 dark:bg-gray-700 dark:text-white`;
    }
  };

  // If the message contains token data, render it as interactive buttons
  if (message.tokens) {
    return (
            ...
            {message.tokens.map((token, index) => (
              <div key={index} className="flex flex-col space-y-1">
                ...
                <div className="text-xs opacity-75">
                  {token.baseCurrency}/{token.quoteCurrency} • {token.markets}
                </div>
              </div>
            ))}
          </div>
      ...
      if (input.toLowerCase().includes('trending') && input.toLowerCase().includes('token')) {
        ...
        setMessages((prev) => [
          ...prev,
          {
            role: "assistant",
            content: aiResponse,
            tokens: trendingTokensData,
            transactionHash: aiRecordHash
          }
        ]);
